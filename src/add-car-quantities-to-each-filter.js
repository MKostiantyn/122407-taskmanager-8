...
export default (filtersData, cardsArray) => {
  if (filtersData && filtersData.size && cardsArray && cardsArray.length) {
    cardsArray.forEach((item, index, array) => {
      ...
      if (!item.dueDate && !isRepeated(item.repeatDays)) {
        filtersData.set(`Today`, filtersData.get(`Today`) + 1);
      } else if (!item.dueDate && isRepeated(item.repeatDays) && item.repeatDays[new Date().getDay()]) {
        filtersData.set(`Today`, filtersData.get(`Today`) + 1);
      } else if (item.dueDate && !isDueDate(item.dueDate) && new Date(item.dueDate).getDay() === new Date().getDay()) {
        filtersData.set(`Today`, filtersData.get(`Today`) + 1);
      }
      if (item.controlButtons && item.controlButtons.get(`Favorites`)) {
        filtersData.set(`Favorites`, filtersData.get(`Favorites`) + 1);
      }
      if (item.controlButtons && item.controlButtons.get(`Archive`)) {
